...
@section('title', __('quiz.create_quiz_title'))
...
<div class="flex items-center gap-3 mb-8">
    ...
    <div>
        <h1 class="text-2xl font-bold text-white">{{ __('quiz.create_quiz_title') }}</h1>
        <p class="text-gray-400 text-sm mt-1">{{ __('quiz.create_quiz_subtitle') }}</p>
    </div>
</div>
...
<div class="max-w-2xl">
    <form method="POST" action="{{ route('admin.quizzes.store') }}" class="space-y-6">
        ...
        <div class="glass rounded-2xl p-6 space-y-5">
            {{-- Title --}}
            <div>
                <label for="title" class="block text-sm font-medium text-gray-300 mb-2">{{ __('quiz.quiz_title') }} *</label>
                <input type="text" name="title" id="title" value="{{ old('title') }}" required
                       ...
                       placeholder="{{ __('quiz.quiz_title_placeholder') }}">
                @error('title')
            ...
            {{-- Description --}}
            <div>
                <label for="description" class="block text-sm font-medium text-gray-300 mb-2">{{ __('quiz.description') }}</label>
                <textarea name="description" id="description" rows="3"
                          ...
                          placeholder="{{ __('quiz.description_placeholder') }}">{{ old('description') }}</textarea>
            </div>

            {{-- Duration & Pass Percentage --}}
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="duration_minutes" class="block text-sm font-medium text-gray-300 mb-2">{{ __('quiz.duration_label') }} *</label>
                    <input type="number" name="duration_minutes" id="duration_minutes" value="{{ old('duration_minutes', 15) }}" min="1" max="300" required
                ...
                <div>
                    <label for="pass_percentage" class="block text-sm font-medium text-gray-300 mb-2">{{ __('quiz.pass_percentage') }} *</label>
                    <input type="number" name="pass_percentage" id="pass_percentage" value="{{ old('pass_percentage', 60) }}" min="0" max="100" required
                           class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary/50 transition-all">
                </div>
            </div>
            ...
            <div class="space-y-3 pt-2">
                <label class="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer">
                    <div>
                        <span class="text-sm font-medium text-gray-200">{{ __('quiz.is_active') }}</span>
                        <p class="text-xs text-gray-500 mt-0.5">{{ __('quiz.is_active_desc') }}</p>
                    </div>
                ...
                <label class="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer">
                    <div>
                        <span class="text-sm font-medium text-gray-200">{{ __('quiz.show_results') }}</span>
                        <p class="text-xs text-gray-500 mt-0.5">{{ __('quiz.show_results_desc') }}</p>
                    </div>
                ...
                <label class="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors cursor-pointer">
                    <div>
                        <span class="text-sm font-medium text-gray-200">{{ __('quiz.randomize_questions') }}</span>
                        <p class="text-xs text-gray-500 mt-0.5">{{ __('quiz.randomize_questions_desc') }}</p>
                    </div>
