...
<div class="space-y-3">
    @foreach($question->options as $option)
        <label class="option-card flex items-center gap-4 group hover:bg-white/10 hover:border-primary/40 cursor-pointer has-[:checked]:bg-primary/15 has-[:checked]:border-primary/50" for="option-{{ $option->id }}">
            <input type="radio"
            ...
            <div class="flex items-center gap-3 flex-1">
                ...
                <span class="text-gray-200 group-hover:text-white transition-colors">{{ $option->option_text }}</span>
            </div>
        </label>
    @endforeach
</div>
